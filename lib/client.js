...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function looksLikeTarget(text, targetLanguage) {
      ...
      //
      // 汉字 + 拉丁字母并存：判"这句话的主体是哪门语言"，**不用比例阈值**。
      //
      // 血泪教训（2026-09-25 实测的连锁故障）：曾用 `汉字占比 >= 0.8`，
      // "比较备份和Web配置文件清单"（11 汉字 / 3 拉丁 = 0.786）刚好卡在阈值下方，
      // 于是这个中文标签被送去翻译；旧宿主对中文原样回显，又被"回显兜底"判成漏译，
      // 结果无限重试、球一直红色。比例阈值在这类"中文夹标识符"上注定拧巴——
      // 同一比例会对应相反的期望（"Hello 世界" 0.29 要翻，"Token 统计" 0.29 可跳）。
      //
      // 改用"拉丁词个数"这个更贴近直觉的量：
      //  - 成句英文会攒出很多拉丁词（Click the button to open the repository → 7 个）；
      //  - 中文标签里混的标识符通常只有 1~2 个（Web、restart.js、pnpm、plugin marketplace）。
      // 于是：拉丁词 >= 4 才认为"英文是主体、需要翻译"；
      // 另外若汉字极少（<= 2）而出现过拉丁词（"Hello 世界"），也算夹心句要翻。
      const latinWords = value.match(/[A-Za-z][A-Za-z0-9_.-]*/g) ?? []
      if (latinWords.length >= 4) return false
      if (hanChars <= 2 && latinWords.length >= 1) return false
      // 其余：中文是主体（含零星英文标识符），已是目标语言。
      return true
    }
    ...
    function createEngine(configOf, refreshConfig) {
      ...
      const stats = {
         ...
         * 本轮没译出来的块/元素。
         *
         * 它们没有 DONE_FLAG，但 `collect` 只在页面变化时才重跑 —— 静置的页面
         * 就再没机会。所以本 pass 结束后要主动为它们再排一轮（见 run()）。
         */
        unresolved: new Set(),
      }
      ...
      async function translateBatch(blocks, signal) {
        ...
        const worker = async () => {
          while (cursor < chunks.length) {
             ...
             * 宿主明确告知"这些条目没翻出来"。
             *
             * 宿主曾经用原文回填这些条目 —— 原文与译文在返回值里长得一样，于是
             * 我们把它当译文写回、还打了 `data-imt-done`，这块内容就再也不会重试，
             * 表现为"漏翻"（2026-09-25 实测：本会话末段一整句英文仍是原文却带 done）。
             * 现在这些 id 必须**保持原文不动、不打 done 标记**，留给下一轮重试。
             */
            const missingIds = new Set(Array.isArray(payload.missing) ? payload.missing.map(String) : [])
            /**
             * 宿主是"支持 missing 字段"的新版吗。
             *
             * 这决定"漏译"如何判定，必须二选一、不能混用：
             *  - 新版：完全信任 `missing`，不再做回显推测。因为合法译文偶尔确实
             *    等于原文（专有名词 "GitHub"、不可译的短串），若在此时也判成漏译，
             *    这些内容会被无谓地反复重试、且永远不打 done。
             *  - 旧版（未重启，无 missing 字段）：它拿不到译文时会**原样回显**我们
             *    发去的文本。此时只能靠推测（见下），否则就会把原文当译文写进页面
             *    并打上 DONE_FLAG —— 这正是用户看到的"漏翻"。
             */
            const hostReportsMissing = Array.isArray(payload.missing)
            // 一个块可能同时有"块条目"和若干"内联元素条目"：先攒齐再统一写回，
            // 否则块条目先写回会把内联元素从 DOM 上摘走，元素条目就落空了。
            const appliedBlocks = new Map()
            /** 有内联元素没译出来的块：整块推迟到下一轮，不打 DONE_FLAG。 */
            ...
            for (const item of chunk) {
              ...
              // 兼容旧宿主（尚未重启）：它拿不到译文时会**原样回显我们发过去的文本**，
              // 且响应里没有 missing 字段。若照旧写回，就把原文当译文写进页面、还打上
              // DONE_FLAG，这块内容永远不再重试 —— 正是"漏翻"的来源。
              // 旧宿主上"原样回显"可能是漏译，也可能是**本来就无需翻译**，必须区分开。
              //
              // 实测（2026-09-25）：旧宿主对"已是中文"的输入会一字不改地返回
              // （"思考用户想要更新…plugin marketplace…" 原样回来）。若把这类也判成
              // 漏译，就会无限重试、永远不打 done、failed 一直大于 0 —— 球变红而内容
              // 其实是对的。反过来，"No argv info there. I have enough…" 这类**整句
              // 纯英文被原样回显**才是用户报的漏翻。
              //
              // 判据：回显值与原文字字相同，且**一个汉字都没有**、还含至少 2 个英文词。
              // 这样"Hello world"（2 词、无汉字）能拦住，而任何带汉字的块都不会被误判。
              // 只用于旧宿主；新宿主有 missing 字段，判定是精确的。
              // 判据：旧宿主把"我们要求翻译的文本"一字不改地还回来，且它还**不像**
              // 目标语言 —— 那就是真漏译。两个附加条件用来排除"本该原样保留"的东西：
              //  ① 必须含空白（是短语/句子）。单个 token（tokenrhythm/deepseek-flashMax、
              //     import-smoke-test.mjs、/Users/... 路径）本就该原样通过，不算失败 ——
              //     否则它们会被永久重试、球一直红色（实测：104 块成功却报"2 块失败"）。
              //  ② 必须不满足 looksLikeTarget：已是中文的标签（"比较备份和Web配置文件清单"）
              //     旧宿主也会原样返回，那是正常行为，不是漏译。
              const echoText = normalize(value)
              // 还要"至少两个英文词"：短夹心串（"Hello 世界"）服务确实可能原样返回，
              // 那是合法结果而非漏译；而"Hello world"、"Click the button…"这类
              // 真句子被原样回显，才说明翻译没生效。
              const echoEnglishWords = (echoText.match(/[A-Za-z]{2,}/g) ?? []).length
              const echoLooksUntranslated = !hostReportsMissing
                && typeof value === 'string'
                && echoText === normalize(item.text)
                && /\s/.test(echoText)
                && echoEnglishWords >= 2
                && !looksLikeTarget(value, language)
              if (echoLooksUntranslated) notifyEchoHost()
              if (missingIds.has(String(item.id)) || echoLooksUntranslated || typeof value !== 'string' || value === '') {
                // 没译出来：一个字都不改，也不打 DONE_FLAG —— 下一轮扫描会重新试。
                stats.failed += 1
                stats.unresolved.add(entry.kind === 'element' ? entry.entry.node : entry.block)
                // 内联元素没译出来时，必须把**整个块**也推迟写回。
                // 否则块条目会照常 applyTo 并打上 DONE_FLAG，而那个内联元素仍是
                // 英文；下轮 collect 见到 DONE_FLAG 就跳过整块，元素永远补不上。
                // 仅译文模式下元素文字由元素条目就地写回，所以只有这个模式需要推迟；
                // 双语模式原文本来就不动，交给块条目处理即可。
                // 注意用独立集合收集、在写回前统一过滤 —— chunk 里元素条目可能排在
                // 块条目之前，直接 delete(appliedBlocks) 会因为顺序而失效。
                if (entry.kind === 'element' && mode !== 'dual') deferBlocks.add(entry.block)
                continue
              }
            ...
            for (const { entry, value } of appliedBlocks.values()) {
              if (deferBlocks.has(entry.block)) {
                // 同块内有内联元素没译出来：整块留到下一轮，避免"英文元素被 done 锁死"。
                stats.unresolved.add(entry.block)
                continue
              }
      ...
      async function run() {
        ...
        // 每轮重新计数：`failed` 的语义是"这一轮有多少条没落上"，不是"从开翻译
        // 到现在累计失败过多少次"。累加会让球在**最终全部成功**之后依然显示红色
        // （实测：106 块已翻对，球却一直是 error，提示条也一直挂着"请重启 DSH"）。
        stats.failed = 0
        emitState()
        ...
        // 本轮有没译出来的块：主动再排一轮。
        // 关键：这些块没有 DONE_FLAG，但 collect 只在页面变化时才重跑，
        // 静置的页面否则就永久留白（用户看到的"漏翻"）。
        if (stats.unresolved.size > 0 && followupPasses < MAX_FOLLOWUP_PASSES) {
          followupPasses += 1
          schedule(400 * followupPasses)
          return
        }
        // 重试用尽：把"确实没能翻出来"的数量定下来，供提示条与球如实显示。
        stats.failed = stats.unresolved.size
        emitState()
        followupPasses = 0
      }
       ...
       * 旧宿主"回显原文"时提示一次。
       *
       * 修复要重启 DSH 让新宿主生效；在此之前客户端只能保证"不把原文当译文锁死"。
       * 提示一次，免得用户以为已经彻底修好。
       */
      let echoNotified = false
      const notifyEchoHost = () => {
        if (echoNotified) return
        echoNotified = true
        // 注意：这里**不能**复用 `imt-stale-host` —— 那个事件的语义是"引擎已被
        // 拦下并停止翻译"（处理函数会 paint({enabled:false}) 并重建菜单）。
        // 而回显兜底是非致命的：翻译仍在继续，只是漏译项不写回。用错事件会让球
        // 变红显示"请重启 DSH"，与实际状态不符（2026-09-25 实测踩到）。
        window.dispatchEvent(new CustomEvent('imt-notice', {
          detail: { message: '宿主漏译时会把原文当译文回显：已跳过这些条目，重启 DSH 后即可正常翻译。' },
        }))
      }
    ...
    function createFloatingBall(engine, refreshConfig, configOf) {
      ...
      // 非致命提示：只弹一句话，不改变球的状态、也不停翻译。
      const onNotice = (event) => {
        const message = event instanceof CustomEvent ? event.detail?.message : undefined
        if (typeof message === 'string' && message !== '') say(message)
      }
      window.addEventListener('imt-notice', onNotice)
      ...
      return {
        ...
        dispose() {
          ...
          window.removeEventListener('imt-notice', onNotice)
