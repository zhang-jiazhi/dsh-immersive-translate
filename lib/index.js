  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
         ...
         * 没拿到译文的条目 id。
         *
         * 这里**不再用原文回填**：原文与真译文在返回值里无法区分，客户端会把
         * "原文"当成译文写回页面、并打上 `data-imt-done` 标记，于是这块内容
         * 永远不再重试 —— 用户看到的就是"漏翻"（2026-09-25 实测：本会话末段
         * `No argv info there.` 整句仍是英文，却带着 done 标记）。
         *
         * 现在改为：**不回填**、id 收进 `missing`，由客户端决定怎么稳妥处理
         * （保持原文不动 + 不打卡片标记 + 下一轮重试）。
         */
        const missing = []
        let failed = 0
        await mapLimit(batches, config.concurrency, async (batch) => {
          ...
          try {
            ...
            for (const item of batch) {
              const value = map.get(item.id)
              if (typeof value === 'string' && value !== '') translations[item.id] = value
              else {
            ...
            if (controller.signal.aborted) return
            failed += batch.length
            for (const item of batch) missing.push(item.id)
            ctx.logger?.warn?.(`[immersive-translate] 批次失败：${error instanceof Error ? error.message : String(error)}`)
          }
        }, controller.signal)
        writeJson(res, 200, { ok: true, translations, missing, failed, total: items.length, language: config.targetLanguage })
      } catch (error) {
        const aborted = controller.signal.aborted
